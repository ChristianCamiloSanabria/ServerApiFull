...
router.get('/showData', async (req, res) => {
    ...
    res.status(200).json(students+","+subjects+","+inscriptions);
});
...
router.get("/show/last_saved_student", async (req, res) => {
    ...
    res.status(200).send(lastStudent);
});
        ...
        listSubjects.forEach(function (element) {
            if (element.id_subject == paramForSearch) {
                ...
                res.status(200).json({
                    ...
                    details: 'Actualizacion ' + newSubject.name_subject
                });
            }
        });
        if (newSubject == null)
            res.status(404).json({
                ...
                details: 'La materia con codigo ' + paramForSearch.code_subject + ' no se encuentra en la base de datos'
            });
        else
...
router.get('/getSubjects', async (req, res) => {
    ...
    if (subjects == null) {
        res.status(404).json({
            ...
            details: 'Base de datos sin elementos'
        });
    }else {
